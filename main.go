...
func main() {
	...
	pushpullServer := newServer(service, logger)
	healthServer := grpc_health.NewServer()

	healthServer.SetServingStatus("pushpull", grpc_health_proto.HealthCheckResponse_SERVING)

	pushpull_proto.RegisterPushPullServer(grpcServer, pushpullServer)
